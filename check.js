...
async function checkBalances() {
  try {
    ...
    const results = await Promise.all(apiIds.map(async (id) => {
      const account = await threeCommasAPI.accountLoadBalances(id);
    ...
    await bot.sendMessage(resultsChatId, 'Balance check results:');
...
bot.onText(/\/check/, (msg) => {
  ...
  // Call the function to check balances and send alerts for api1
  checkBalances();
});
